...
int main(int argc, char *argv[]) 
{
	...
	if(mode == 0){
		...
		/*-10 is out of the range possible for temperature. The server will know that this actuator wants to unsubscribe*/
		actuator.temp = -10;
		...
		printf("Insert Sensors to listen: 	(Terminate with empty string)\n");
		itemType sensor;
		do{
			scanf("%s", sensor.id);
			if(!strcmp(sensor.id, ""))
				sensorsToListen = EnqueueFirst(sensorsToListen, sensor);
		}while(!strcmp(sensor.id, ""));
		...
		if ( send(sockfd, getLength(sensorsToListen), sizeof(int), 0) == -1 ) 
		{
			perror("Error on send\n");
			exit(1);
		}
		...
		for(int i = 0; i < getLength(sensorsToListen); i++){
			if ( send(sockfd, &tmp_list, sizeof(itemType), 0) == -1 ) 
			{
				perror("Error on send\n");
				exit(1);
			}
		...
		while(1){
			...
			if(strcmp(msg.id, "")){
				...
				printf("---Received temperature: %f from Sensor: %s\n", msg.temp, msg.id);
				...
				for(int i = 0; i < getLength(sensorsToListen); i++){
					if(tmp_list->item.temp > actuator.temp){
						onOffSwitch++;
					}else{
						onOffSwitch--;
					}
